...
One Next.js application, one deployment, one domain. The public site now presents
Actprove as a drone software and hardware company. PROVE-1 is the ground control
and analysis platform; ACT-1 is the onboard compute and perception system. The
site also presents the Savlo Dynamics collaboration and the planned ARCA-1,
DETECT-1, and ERA-1 systems.

The protected `/admin-pro` flight-performance console and Python solver are
legacy engineering tools from the earlier airframe direction. They remain in the
repository and were not redesigned as part of the public-site update.
